...
\textbf{Boosting} AdaBoost is a short name for ``adaptive boosting''. In theory, boosting can be used to significantly reduce the error of ``weak'' classfiers. AdaBoost can be seen as a broad extension of on-line prediction model in a general decision-theoretic setting. By adapting the multiplicative weight-update rule, AdaBoost is adaptive and can turn weak classifers into a strong classifier.

%It has been demonstrated that Ada-Boosting can be viewed as a form of additive modeling for optimizing a logistic
%loss function. 
